        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="relative">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search users..." className="pl-9 pr-4 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm outline-none focus:border-[tomato] text-slate-900 dark:text-white placeholder:text-slate-400" />
            </div>
          </div>
                      ...
                      <td className="py-3 px-2">
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            title="Edit"
                          >
                            <Edit2 className="w-4 h-4 text-slate-900 dark:text-white placeholder:text-slate-400" />
                          </button>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {txs.slice(0, 50).map((t) => (
                <div key={t.id} onClick={() => setSelectedTx(t)} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer transition">
                  <div>
                    <p className="font-semibold text-sm text-slate-900 dark:text-white placeholder:text-slate-400">{t.sender_name} → {t.receiver_name}</p>
        ...
        {editing && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setEditing(null)}>
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
              <h3 className="text-lg font-bold mb-4 text-slate-900 dark:text-white placeholder:text-slate-400">Manage User — {editing.name}</h3>
              <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">
                  Upgrade Balance
              </label>
              <input type="number" value={newBal} onChange={(e) => setNewBal(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 outline-none focus:border-[tomato] text-slate-900 dark:text-white placeholder:text-slate-400" />
              <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">
                  Account Number
              </label>
              <input type="text" value={newAccountNumber} onChange={(e) => setNewAccountNumber(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 outline-none focus:border-[tomato] text-slate-900 dark:text-white placeholder:text-slate-400" />
              {/* ROLE SELECT */}
              <div className="mt-4">
                <label className="block text-sm font-semibold text-slate-600 dark:text-slate-300 mb-2">
                  User Role
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 outline-none focus:border-[tomato] font-semibold text-slate-900 dark:text-white placeholder:text-slate-400"
                >
              ...
              <div className="flex gap-2 mt-4">
                <button onClick={() => setEditing(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 font-semibold text-slate-900 dark:text-white placeholder:text-slate-400">Cancel</button>
              <button
                onClick={saveBalance}
